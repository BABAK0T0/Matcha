...
const insertMatched = (userone, usertwo) => {
	...
	return new Promise((resolve, reject) => {
		connection.query(sql, [userone, usertwo, today], (e, success) => (e) ? reject(e) : resolve(success))
	})
}
...
const getListToMatch = (criteria) => {
	...
	sortBy = (sortBy == "age") ? 'birthdate' : sortBy
	sortBy += (sortBy == "age" || sortBy == "score") ? ' DESC' : ''
...
const getDefaultListToMatch = (criteria) => {
	...
	let dateMin = tmpymd
	let dateMax = (parseInt(ymd[2]) + interAge) + '-' + ymd[1] + '-' + ymd[0]
	let sortBy = "distance"
